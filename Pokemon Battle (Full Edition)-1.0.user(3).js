...
function renderHeader() {
  ...
  row.appendChild(createButton('Settings ⚙️', openSettings)); // Only control menu needed here
...
function scheduleRandomBattle() {
  ...
  randomBattleTimer = setTimeout(() => {
    ...
    scheduleRandomBattle(); // Chain next random battle
  }, delay);
}
    ...
    onload(res) {
      ...
      GM.xmlHttpRequest({
        ...
        onload(evRes) {
          ...
          // Traverse the chain to find current species
          while (current && !found) {
          ...
          if (found && current.evolves_to.length > 0) {
            ...
            const requiredLevel = evoDetails?.min_level || 0;
...
info.innerHTML += `<br>Catch Chance: ${(chance * 100).toFixed(1)}%`;
  ...
  battlePanel.appendChild(ctl);
    // ✅ Add Sleep Powder button after main options
...
function throwBall() {
  const useBall = prompt("Which ball? (poke, great, ultra)", "poke").toLowerCase();
